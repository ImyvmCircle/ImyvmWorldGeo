...
1. 说什么执行什么。
2. 没说的不执行。
3. 执行任何修改、写作或结构调整前，先向用户完整叙述本轮方案，并在会话内以问题确认；用户确认后继续执行，不以等待确认为由结束对话。
4. 不确定就问，别猜；发现缺失文件、依赖 API、服务器设定、项目意图或设计框架时先询问。
5. 给验收标准，别给步骤。
6. 执行修改任务前先检查远端仓库主分支状态，并在不发生冲突时尽可能同步；冲突交由用户裁决。
7. 不主动使用 git；prompt 要求 git、提交、推送、拉取或发布时，先同步远端再操作。
8. 完成后给一条符合项目既有格式的 commit 描述建议。
9. 不自行更新 tag、版本号或发布版本。
10. 原则上不新建 class，不添加 comments，除非 prompt 要求。
