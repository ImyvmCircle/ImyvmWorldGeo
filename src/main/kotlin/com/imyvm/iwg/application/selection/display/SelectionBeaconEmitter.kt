...
// Large bright-white dust particles (size 4.0 = max visible radius, ~4× bigger than END_ROD).
// Pillar step = 3 blocks (much denser than the previous step-8 approach).
// longDistance=true extends render to 512 blocks.
...
fun beginBeaconPillarTracking() {
    pendingPillarPositions.get().clear()
}

fun emitBeaconPillar(player: ServerPlayerEntity, x: Int, z: Int) {
    val world = player.serverWorld
    if (!world.chunkManager.isChunkLoaded(x shr 4, z shr 4)) return
    pendingPillarPositions.get().add(x to z)
}

fun commitBeaconPillars(player: ServerPlayerEntity) {
    val world = player.serverWorld
...
fun clearBeaconBeams(@Suppress("UNUSED_PARAMETER") player: ServerPlayerEntity) = Unit
