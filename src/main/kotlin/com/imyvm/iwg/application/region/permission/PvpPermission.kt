...
fun playerPvpPermission() {
    AttackEntityCallback.EVENT.register { player, world, hand, entity, hitResult ->
        ...
        val target = entity
        val resolved = RegionDatabase.getRegionScopeSubSpaceAt(world, target.blockPosition().x, target.blockPosition().z)
        resolved?.let { (region, scope, subSpace) ->
            val attackerDenial = findPermissionDenial(region, scope, subSpace, player.uuid, PermissionKey.PVP, PERMISSION_DEFAULT_PVP.value)
            val defenderDenial = findPermissionDenial(region, scope, subSpace, target.uuid, PermissionKey.PVP, PERMISSION_DEFAULT_PVP.value)
            if (attackerDenial != null || defenderDenial != null) {
                val ctx = (attackerDenial ?: defenderDenial!!).context()
                val messageKey = when {
    ...
    ServerLivingEntityEvents.ALLOW_DAMAGE.register { entity, source, _ ->
        ...
        val target = entity
        val resolved = RegionDatabase.getRegionScopeSubSpaceAt(entity.level(), target.blockPosition().x, target.blockPosition().z)
        resolved?.let { (region, scope, subSpace) ->
            val attackerDenial = findPermissionDenial(region, scope, subSpace, attacker.uuid, PermissionKey.PVP, PERMISSION_DEFAULT_PVP.value)
            val defenderDenial = findPermissionDenial(region, scope, subSpace, target.uuid, PermissionKey.PVP, PERMISSION_DEFAULT_PVP.value)
            if (attackerDenial != null || defenderDenial != null) {
                val ctx = (attackerDenial ?: defenderDenial!!).context()
                val messageKey = when {
