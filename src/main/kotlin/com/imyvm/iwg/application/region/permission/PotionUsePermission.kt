...
fun playerPotionUsePermission() {
    UseItemCallback.EVENT.register { player, world, hand ->
        ...
        if (!stack.isOf(Items.SPLASH_POTION) && !stack.isOf(Items.LINGERING_POTION)) return@register TypedActionResult.pass(stack)
        ...
        regionAndScope?.let { (region, scope) ->
            ...
            if (denial != null) {
                if (hand == Hand.MAIN_HAND) {
                    player.sendMessage(Translator.tr("setting.permission.potion_use", buildPermissionDenialContext(region, scope, denial)))
                }
