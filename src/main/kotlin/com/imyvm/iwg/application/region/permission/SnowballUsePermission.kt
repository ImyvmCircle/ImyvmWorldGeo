...
fun playerSnowballUsePermission() {
    UseItemCallback.EVENT.register { player, world, hand ->
        ...
        if (!stack.isOf(Items.SNOWBALL)) return@register TypedActionResult.pass(stack)
        ...
        regionAndScope?.let { (region, scope) ->
            ...
            if (denial != null) {
                if (hand == Hand.MAIN_HAND) {
                    player.sendMessage(Translator.tr("setting.permission.snowball_use", buildPermissionDenialContext(region, scope, denial)))
                }
