...
class WorldGeoConfig : HokiConfig("Imyvm_world_geo.conf") {
    companion object{
        ...
        @JvmField
        @ConfigOption
        val PERMISSION_DEFAULT_FARMING = Option(
            "core.permission.farming.default",
            true,
            "the default farming (breaking and planting crops on farmland) permission in regions."
        ) { obj: Config, path: String? ->
            obj.getBoolean(path)
        }
        ...
        @JvmField
        @ConfigOption
        val SELECTION_MAX_POINTS = Option(
            "core.selection.max_points",
            12,
            "the maximum number of points a player can hold in selection mode. Adding a point beyond this limit is rejected."
        ) { obj: Config, path: String? ->
            obj.getInt(path)
        }
        ...
        @JvmField
        @ConfigOption
        val SELECTION_MIN_POINTS = Option(
            "core.selection.min_points",
            1,
            "the minimum number of points that must remain in selection mode after an undo. Undoing below this limit is rejected."
        ) { obj: Config, path: String? ->
            obj.getInt(path)
        }
    }
